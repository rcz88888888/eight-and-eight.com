...
const updateParallax = () => {
  ...
  const backgroundSpeeds = [0.012, 0.016, 0.02, 0.024, 0.028, 0.032, 0.036, 0.04];
  backgroundSpeeds.forEach((speed, index) => {
    document.documentElement.style.setProperty(
      `--background-shift-${index + 1}`,
      `${(scrollY * -speed).toFixed(2)}px`
    );
  });
